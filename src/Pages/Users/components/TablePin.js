...
import { setPinId } from '../../../redux/feature/pin/pinSlice';
...
export default function TablePin() {
    ...
    const [idPin, setIdPin] = React.useState(null);
    ...
    const columns = [
        ...
        { field: 'usedby', headerName: 'usedBy', hide: true },
        { field: 'no', headerName: 'No', hide: false, align: "center", flex: 0.1 },
        { field: 'pin', headerName: 'Pin', hide: false, align: "center", flex: 1 },
        { field: 'whouse', headerName: 'Digunakan Oleh', hide: false, align: "center", renderCell: (params) => {
            if(params.row.usedby == null){
                return(
                    <p>Belum Digunakan</p>
                )
            }else{
                return(
                    <p>{params.row.usedby.firstName} {params.row.usedby.lastName}</p>
                )
            }
        }, flex: 1 },
        { field: 'timeused', headerName: 'Tanggal Digunakan', hide: false, align: "center", flex: 1 },
        { field: 'detail', headerName: 'Detail', hide: false, align: "center", flex: 1, renderCell: (params) => {
            return(
                <Button variant='outlined' size='small' onClick={() => setIdPin(params.row.id)}>
                    Lihat Detail
                </Button>
            )
        } },
    ...
    const rows = pinList.map((data, index) => {
        var numb = index + 1;
        ...
        return {
            ...
            pin: data.pins,
            usedby: data.usedby,
            timeused: new Date(data.updatedAt)
